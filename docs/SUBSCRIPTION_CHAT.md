...
Codex 订阅 Chat 支持文本以及 JPEG、PNG、WebP、GIF 图片，图片通过 Codex 原生输入发送，
继续使用 Codex 额度。恢复历史时会把图片还原为图片输入，原生会话续聊不重复发送历史附件。
其他订阅连接目前只支持文本，非图片文件可使用支持相应附件的 API Profile 或开发任务处理。
调用失败的具体原因保存在聊天记录中，重新打开对话后仍可查看。
用量展示不把未知值伪装成零；各家 CLI 未统一提供精确额度接口。
