...
文件接受 `# User Preferences` 或 `# 用户偏好` 标题及单行列表。上限为完整文件 6,000 Unicode 字符、30 条偏好；手动整理可附一个 `## Last Consolidation` 小节，上限 1,500 字符、模型至多五条短状态，每条最多 200 字符且不能包含换行。这些单行限制同时写入模型的 JSON Schema 并在接收时校验。字符预算不是所有 provider 的 token 计量。

模型返回的 JSON 或字段格式不合法时，DreamAgent 保留当前块的原始证据，将具体校验错误交给模型重新生成，最多尝试三次（首次请求加两次纠正）。API 与本地 harness 共用这条规则，每次请求仍检查输入预算；不直接截断内容或接受部分 JSON。连续失败保留已完成块且不发布部分记忆，下次整理可从断点继续。连接故障、取消、输出截断及发布错误不进入这条格式纠正循环。
...
整理和 Git 历史操作需要系统已安装 Git，且 Cleo 启动时的 PATH 能找到 `git` 命令。桌面发布包不另行捆绑 Git；没有配置远端或全局 Git 作者也可以使用本地记忆版本管理。
