...
命令执行器将正常输出与诊断分开：JSON、提交 SHA 和源码文件列表只使用 stdout，
stderr 继续进入日志及失败诊断。源码文件枚举保留 NUL 分隔符和文件名空格；出现读取警告时
明确报告枚举不完整，不把警告当作文件名计算摘要，也不继续导出不完整快照。
本地测试生成的 `tests/.tmp-*/` 与 `tests/.diagnostic-deps/` 不参与源码扫描或程序源码打包。
...
冻结案例保存在源码目录外的 `acceptance/suite.json`。`dream-format` 在独立临时 home 中
运行基线和候选版本的实际 extractor，以相同的无效/纠正响应验证格式恢复与证据保留。
回放禁止网络，只证明该测试场景，不代表真实模型质量、检索准确性或真实记忆发布成功，
也不等于操作系统级沙箱。构建检查和行为结果保持独立，源码或案例变化会使旧结果失效。
