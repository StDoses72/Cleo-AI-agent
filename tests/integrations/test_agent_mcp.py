...
def test_runtime_argument_is_hidden_and_injected(monkeypatch):
    ...
    async def exercise():
        async with Client(server) as client:
            ...
            schema = listed[0].model_dump(by_alias=True)["inputSchema"]
            assert "runtime" not in schema["properties"]
            result = await client.call_tool("probe", {})
            assert result.content[0].text == "thread-42"
...
def test_dream_server_rejects_cross_source_tool_arguments(monkeypatch):
    ...
    async def exercise():
        async with Client(server) as client:
            result = await client.call_tool("remember", scope)
            assert result.content[0].text == "accepted"
            for key in scope:
                with pytest.raises(ToolError, match="cannot change"):
                    await client.call_tool("remember", {**scope, key: "elsewhere"})
