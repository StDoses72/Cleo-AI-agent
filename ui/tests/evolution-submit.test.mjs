...
test("main and omitted targets are rejected before any remote action", async (t) => {
  ...
  for (const targetBranch of [undefined, "main", "refs/heads/main", "MAIN", " main "]) {
    await assert.rejects(manager.submitPullRequest(title, body, randomUUID(), { targetBranch, buildId: "saved" }), /目标分支|main/);
  }
...
async function fixture(t) {
  ...
  return { manager, root, remote, calls, command, tools, remotePrs, branches, issues };
}
...
test("branch application waits for a maintainer-created target and then submits the selected version from a fork", async (t) => {
  ...
  assert.ok(!f.calls.some(({ args }) => args.includes("merge") || args.includes("--force") || args.includes("--force-with-lease")));
});
