...
async function fixture(action) {
  ...
  const manager = {
    ...
    async runCommand(command, args) {
      ...
      if (path.includes("/git/commits/")) return JSON.stringify({ sha: remote.badCommit ? "b".repeat(40) : remote.commit, tree: { sha: "c".repeat(40) } });
      if (path.includes("/git/trees/")) return JSON.stringify({ sha: remote.badTree ? "d".repeat(40) : "c".repeat(40), truncated: remote.truncated,
        tree: [{ path: "README.md", type: "blob", mode: remote.mode || "100644", sha: remote.blob }] });
...
for (const local of ["different-active", "dirty", "unprepared", "pruned"]) {
  test(`one-click merged PR release works with ${local} source without touching user state`, async () => {
    await fixture(async ({ manager, remote, state }) => {
      ...
      if (local === "pruned") state.builds = [];
      const original = structuredClone(state);
      const bytes = await readFile(join(manager.source, "README.md"));
      manager.prepareTools = async () => assert.fail("Must not prepare the current workspace");
      manager.sourceHash = async () => assert.fail("Must not read the current workspace");
      const result = await publishMergedRelease(manager, { url, tag: "v1.2.3" });
      assert.equal(result.commit, commit);
      assert.equal(result.title, "v1.2.3"); assert.equal(result.body, ""); assert.equal(result.prerelease, false);
      assert.equal(remote.writes[0].payload.sha, commit);
      assert.equal(remote.writes[1].payload.target_commitish, commit);
      assert.equal(remote.writes[1].payload.name, "v1.2.3");
      assert.equal(remote.writes[1].payload.prerelease, false);
      assert.deepEqual(state, original);
      assert.deepEqual(await readFile(join(manager.source, "README.md")), bytes);
      await publishMergedRelease(manager, { url, tag: "v1.2.3" });
      assert.equal(remote.writes.length, 2, "Retry reconciles rather than recreates");
    });
  });
}

for (const failure of ["empty", "invalid", "unmerged", "branch", "commit-object", "tree-object", "truncated", "permission", "account"]) {
  test(`one-click rejects ${failure} before any tag/release write`, async () => {
    await fixture(async ({ manager, remote }) => {
      const params = { url, tag: "v1.2.3", login: "owner" };
      if (failure === "empty") params.tag = "";
      if (failure === "invalid") params.tag = "abc";
      if (failure === "unmerged") remote.merged = false;
      if (failure === "branch") remote.branch = "wrong-target";
      if (failure === "commit-object") remote.badCommit = true;
      if (failure === "tree-object") remote.badTree = true;
      if (failure === "truncated") remote.truncated = true;
      if (failure === "permission") remote.push = false;
      if (failure === "account") remote.login = "changed";
      await assert.rejects(publishMergedRelease(manager, params));
      assert.equal(remote.writes.length, 0);
    });
  });
}
...
test("new source selection resolves its own merge commit and ignores client commit/branch-head hints", async () => {
  await fixture(async ({ manager, remote, state }) => {
    ...
    assert.equal(remote.tags["v1.2.4-beta.1"], remote.commit);
    assert.equal(remote.releases["v1.2.4-beta.1"].prerelease, true);
  });
});
...
test("one-click response loss is recoverable and duplicate concurrent submissions remain serialized", async () => {
  await fixture(async ({ manager, remote }) => {
    ...
    remote.responseLost = true;
    const params = { url, tag: "v1.2.3" };
    const first = publishMergedRelease(manager, params);
    await assert.rejects(publishMergedRelease(manager, params), /operation/);
    assert.match((await first).releaseUrl, /v1\.2\.3$/);
    assert.equal(remote.writes.length, 2);
  });
});

test("package publication keeps using the verified remote PR source after a one-click release", async () => {
  await packageFixture(async ({ manager, params, actions, state }) => {
    params.sourceKind = "merged-pr";
    state.prepared = false; state.draftDirty = true; state.active = "other"; state.builds = [];
    manager.sourceHash = async () => assert.fail("Remote PR package publication must not read local edits");
...
test("partial-draft recovery can inspect its remote PR source without creating a release", async () => {
  await fixture(async ({ manager, remote, state }) => {
    state.prepared = false; state.draftDirty = true; state.builds = [];
    const before = structuredClone(state);
    const source = await previewMergedRelease(manager, { url });
