...
export function App() {
  ...
  /** Save feedback once, then route both clarification and implementation through preparation. */
  const improveFromCase = async (caseId: string, body: string, id: string) => {
    setEvolutionOpen(true);
    const thread = evolutionThread ? workspace.activeThread : await startEvolution();
    if (!thread) throw new Error("无法打开进化会话，反馈尚未发送。");
    const request = await evolution.run<EvolutionRequest>("feedbackRequest", { id, caseId, body, threadId: thread.id });
    if (request.status === "frozen" && !request.execution) {
          ...
          header={evolutionOpen ? <EvolutionPanel state={evolution.state} error={evolutionIssue || evolution.error}
            ...
            onAction={evolutionAction} onRetry={() => retryEvolution.current()} onRepair={() => { void repairEvolution(); }}>
              <EvolutionCases state={evolution.state?.acceptance} busy={evolution.pending || Boolean(workspace.runningThreadId) || evolution.state?.phase !== "idle"}
                ...
                key={`${evolution.state?.acceptance?.report?.sourceHash}:${evolution.state?.acceptance?.report?.createdAt}:${evolution.state?.active}`}
                ...
                canReview={Boolean(evolution.state?.candidate && evolution.state.active === evolution.state.candidate)}
