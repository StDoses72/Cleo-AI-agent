...
const forbidden = (value: string) => value.trim().replace(/^refs\/heads\//, "").toLowerCase() === "main";
...
export function EvolutionContribution({ state, busy, onAction, onBusy, onSubmitted }: Props) {
  ...
  const submit = async () => {
    ...
    if (!target.trim() || forbidden(target)) { setError("请选择或填写非 main 的目标分支。"); return; }
  ...
  return <section aria-label="贡献提交">
    ...
    <form onSubmit={(e) => { e.preventDefault(); void submit(); }}>
      {mode === "existing" ? <>
        ...
        <p>这是通过 GitHub Issue 提交的申请，不代表分支已创建。owner/collaborator 创建后，请检查状态，再单独提交 PR。</p>
      ...
      {forbidden(target) && <p role="alert">不允许以 main 为目标分支。</p>}
    ...
    {Boolean(state?.branchRequests?.length) && <div className="evolution-pr-history" aria-label="目标分支申请">
      ...
      {state?.branchRequests?.filter((request) => request.url).map((request) => <article key={request.id}>
        <div><a href={request.url} target="_blank" rel="noreferrer">{request.branch}</a>
          <p>{request.status === "ready" ? "已确认分支存在，可以提交 PR（提交时再次核对）" : "申请已提交，尚未确认分支创建"}</p>
