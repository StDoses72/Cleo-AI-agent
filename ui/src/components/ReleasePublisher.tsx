...
export function ReleasePublisher({ state, busy, initialUrl = "", buildId, onAction, onBusy }: Props) {
  ...
  return <section className="release-publisher" aria-label="创建 GitHub Release">
    ...
    {!prepared && <p>请先切换到该 PR 对应的本地版本，准备并核验源码。</p>}
    ...
    {result && <p role="status">{result.prerelease ? "预发布版" : "正式版"} {result.tag} 已发布。<a href={result.releaseUrl} target="_blank" rel="noreferrer">查看 GitHub Release</a></p>}
  </section>;
}
