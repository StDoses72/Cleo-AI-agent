...
export function EvolutionCases({ state, thread, busy, canCompare, onAction, onImprove, onCreate }: Props) {
    ...
    {state && Boolean(cases.length) && <details className="evolution-case-list"><summary>查看预期和修改前后结果</summary>
      ...
      {cases.map((item) => {
        ...
        return <article key={item.id} className="evolution-case">
          <div><b>{item.title}</b><small>{item.kind === "dream-format" ? "自动 · Dream 格式恢复" : "人工 · 对话行为"}</small></div>
          <p>{item.expectation}</p>
          <div className="evolution-comparison">
            <div><small>修改前</small><strong>{result ? labels[result.before.status] : "待比较"}</strong><p>{result?.before.detail}</p></div>
