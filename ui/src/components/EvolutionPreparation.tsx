    ...
    {requests.map((request, index) => <article key={request.id} className="evolution-request">
      ...
      {request.cases.map((detail) => {
        ...
        return <details key={item.id} className="evolution-case" open={isCurrent && active !== false}>
          <summary>{item.title} · {completed ? "已验收" : active === false ? "历史预期" : isCurrent && !request.repair ? "本轮新增" : item.kind === "manual" ? "待完成" : "回归案例"}</summary>
          <dl><dt>对应要求</dt><dd>{detail.requirement}</dd><dt>当前行为</dt><dd>{detail.current}</dd>
            ...
            <dt>验证方式与结果</dt><dd>{item.kind === "dream-format" ? "自动 · Dream 格式回放" : "人工验收"} · {
              completed ? (completed.note ? `已验收：${completed.note}` : "用户已确认验收") : acceptance?.fresh && result?.status === "passed" ? "通过" : acceptance?.fresh && result?.status === "failed" ? "未通过" : acceptance?.fresh && result?.status === "error" ? "运行失败" : item.kind === "manual" ? "待人工验收" : "待回放"}</dd></dl>
