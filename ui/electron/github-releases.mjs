...
async function releaseSource(manager, tools, url) {
...
async function tagCommit(manager, tools, tag) {
...
export async function publishRelease(manager, params) {
  return manager.operation("publishing", async () => {
    ...
    const post = async (path, payload) => {
      ...
      return JSON.parse(await manager.runCommand(tools.gh, ["api", "--method", "POST", path, "--input", file], { env: tools.env }));
    };
    try {
      ...
      const releasePath = `${endpoint}/releases/tags/${encodeURIComponent(tag)}`;
      let result = await optionalApi(manager, tools, releasePath);
      if (!result) {
        ...
        catch (error) { result = await optionalApi(manager, tools, releasePath); if (!result) throw error; }
      }
      if (result.tag_name !== tag || result.prerelease !== prerelease || result.draft
      ...
      return { ...source, tag, prerelease, releaseUrl: result.html_url };
