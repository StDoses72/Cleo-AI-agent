...
app.whenReady().then(async () => {
  ...
  ipcMain.handle("cleo:update:install", () => updater.install(async (version) => {
  ...
  ipcMain.handle("cleo:evolution:action", async (_event, payload) => {
    ...
    if (backend.pending.size && ["prepare", "build", "merge", "submit", "apply", "recovery", "select", "discard", "save", "begin", "repairPrompt", "createCase", "archiveCase", "compareCases", "reviewCase", "prepareRequest", "repairRequest", "reviseRequest", "feedbackRequest", "completeCase"].includes(action)) {
